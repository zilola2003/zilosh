...
st.title("Diabet natijasini bashorat qilish")

# Foydalanuvchidan ma'lumotlarni kiritishni so'rash
pregnancies = st.number_input("Homiladorlik sonini kiriting", min_value=0, max_value=20, value=1, step=1)
...
diabetes_pedigree_function = st.number_input("Diabet Pedigree Funksiyasini kiriting", min_value=0.0, max_value=3.0, value=0.5, step=0.01)
age = st.number_input("Yoshni kiriting", min_value=0, max_value=120, value=25, step=1)

# Modelni yuklash va bashorat qilish
if st.button("Diabetni bashorat qilish"):
    ...
    # Modelni yuklash
    model = joblib.load('decision_tree_model.pkl')  # Model faylingiz nomini 'diabetes_model.pkl' ga moslang
    ...
    # Natijani ko'rsatish
    if outcome[0] == 1:
        st.write("Ushbu bemorda diabet rivojlanish ehtimoli bor.")
    else:
        st.write("Ushbu bemorda diabet rivojlanish ehtimoli yo'q.")
